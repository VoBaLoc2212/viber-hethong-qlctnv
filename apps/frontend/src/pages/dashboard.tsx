...
import { useGetDashboardKpis, useGetExpensesByMonth, useGetTransactions } from "@workspace/api-client-react";
          ...
          <CardContent className="px-2 sm:px-6 h-[350px]">
            {isLoadingChart ? (
              ...
              <div className="h-full flex items-center justify-center text-muted-foreground flex-col">
                <img src={`${import.meta.env.BASE_URL}images/empty-state.png`} alt="No data" className="w-32 h-32 opacity-50 mb-4" />
                  ...
                  <div key={tx.id} className="p-4 flex items-center justify-between hover:bg-secondary/30 transition-colors">
                    ...
                    <div className="text-right">
                      ...
                      <Badge variant="outline" className={`text-[10px] mt-1 ${
                        tx.status === 'APPROVED' ? 'border-green-200 text-green-600 bg-green-50' :
                        tx.status === 'REJECTED' ? 'border-red-200 text-red-600 bg-red-50' :
...
function KpiCard({ title, value, icon, loading, valueClass = "" }: { title: string, value?: number, icon: React.ReactNode, loading: boolean, valueClass?: string }) {
  return (
